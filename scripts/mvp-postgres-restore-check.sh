...
EXPECTED_REVISION="${OPENSTORYLINE_EXPECTED_SCHEMA_REVISION:-20260717_0001}"
...
local_restore_check() {
  ...
  table_count="$(psql --dbname "$database" --no-align --tuples-only --command \
    "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('auth_sessions', 'login_attempt_buckets', 'editing_sessions', 'video_jobs', 'job_events', 'artifacts', 'audit_documents', 'audit_reviews')")"
...
table_count=\$(psql --username \"\$POSTGRES_USER\" --dbname \"\$database\" --no-align --tuples-only --command \"SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('auth_sessions', 'login_attempt_buckets', 'editing_sessions', 'video_jobs', 'job_events', 'artifacts', 'audit_documents', 'audit_reviews')\")
