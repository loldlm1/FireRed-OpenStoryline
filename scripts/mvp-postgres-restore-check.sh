...
EXPECTED_REVISION="${OPENSTORYLINE_EXPECTED_SCHEMA_REVISION:-20260719_0002}"
...
local_restore_check() {
  ...
  table_count="$(psql --dbname "$database" --no-align --tuples-only --command \
    "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('auth_sessions', 'login_attempt_buckets', 'editing_sessions', 'session_input_videos', 'prompt_versions', 'video_jobs', 'job_events', 'artifacts', 'audit_documents', 'audit_reviews')")"
